#!/bin/bash
#
# ## Plugin to check horizon supervisor metrics
# ##
# ## Changes:
...
#
#
# Usage: ./check-horizon-supervisor-metrics.sh --hostname=api.yanovis.com --identifier=high,events,default
         #--path=
         #--warning-queued=
         #--critical-queued=
         #--warning-waittime=
...
# ## Output:
#
# OK - All metrics ok | '<identifier>.queued'=0;0:100; '<identifier>.waittime'=0;; '<identifier>.processes'=12;;
# OK - All metrics ok | 'high,events,default.queued'=0;0:100; 'high,events,default.waittime'=0;; 'high,events,default.processes'=12;;
#
# Exit Codes
# 0 OK       .. is ok
...
threshold_warn_processes=""
threshold_crit_processes=""
...
while test -n "$1"; do
  ...
  --critical-waittime=*)
    ...
    ;;
  --warning-processes=*)
    threshold_warn_processes="${1#*=}"
    ;;
  --critical-processes=*)
    threshold_crit_processes="${1#*=}"
    ;;
  *)
...
queued=$(echo "$json" | jq -r ".supervisors[\"$identifier\"].queued_jobs")
waittime=$(echo "$json" | jq -r ".supervisors[\"$identifier\"].wait_time")
processes=$(echo "$json" | jq -r ".supervisors[\"$identifier\"].processes")
...
processes_perf_part="'${identifier}.processes'=${processes};;"

perf_output="${queued_perf_part} ${waittime_perf_part} ${processes_perf_part}"
...
if [ "$processes" == "null" ]; then
  ((warning++))
  alarms+=("Running processes - $processes")
elif check_in_range "$processes" "$threshold_crit_processes"; then
  ((critical++))
  alarms+=("Running processes - $processes")
elif check_in_range "$processes" "$threshold_warn_processes"; then
  ((warning++))
  alarms+=("Running processes - $processes")
fi
