...
package network::brocade::foundry::snmp::mode::components::temperature;

use strict;
use warnings;

my $mapping = {
    sysChassisTempTemperature => { oid => '.1.3.6.1.4.1.1991.1.1.1.4.1.1.4' },
};
...
sub check {
    ...
    foreach my $oid ($self->{snmp}->oid_lex_sort(keys %{$self->{results}->{$mapping->{sysChassisTempTemperature}->{oid}}})) {
        ...
        $result->{sysChassisTempTemperature} *= 0.5;
