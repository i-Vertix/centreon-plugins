...
package network::brocade::foundry::snmp::mode::uptime;
...
sub manage_selection {
    ...
    my $oid_agentSerialNumber = '.1.3.6.1.2.1.47.1.1.1.1.11.101';
    my $oid_agentFirmware = '.1.3.6.1.2.1.16.19.2.0';

    if (defined($self->{option_results}->{add_sysdesc})) {
        my $snmp_result = $options{snmp}->get_leef(
            oids => [ $oid_agentSerialNumber, $oid_agentFirmware ],
            nothing_quit => 1
        );
