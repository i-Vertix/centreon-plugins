...
package network::paloalto::prisma::sase::restapi::mode::serviceconnectionstatus;
...
sub prefix_service_connections_output {
    my ($self, %options) = @_;

    return sprintf(
        "Service Connection Site %s [location %s - region %s]",
    ...
    $self->{maps_counters_type} = [
        {
            ...
            type             => 1,
            cb_prefix_output => 'prefix_service_connections_output',
            message_multiple => 'All service connections are ok'
        }
    ];

    $self->{maps_counters}->{service_connections} = [
        {
            label            => 'status',
            type             => 2,
...
sub manage_selection {
    ...
    foreach my $service_connection (@{$response->{data}}) {
        next if (defined($self->{option_results}->{filter_site_name}) && $self->{option_results}->{filter_site_name} ne '' &&
            $service_connection->{site_name} !~ /$self->{option_results}->{filter_site_name}/);

        $self->{service_connections}->{ $service_connection->{site_name} } = {
            site_name          => $service_connection->{site_name},
            site_state         => $service_connection->{site_state_name},
            edge_location_name => $service_connection->{edge_location_name},
            cloud_region_name  => $service_connection->{cloud_region_name},
            avg_egress         => $service_connection->{avg_egress} * 1000000,
            avg_ingress        => $service_connection->{avg_ingress} * 1000000,
            avg_throughput     => $service_connection->{avg_throughput} * 1000000,
            peak_throughput    => $service_connection->{peak_throughput} * 1000000,
            site_all_tunnels   => $service_connection->{site_all_tunnels},
            site_up_tunnels    => $service_connection->{site_up_tunnels}
        };
    }
}
